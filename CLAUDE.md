...
- `analytics_db.py` exposes **two separate connection pools**: `get_pool()` (large, read-serving, used by every descriptive/predictive/prescriptive endpoint) and `get_etl_pool()` (small, capped) for ETL/sync writers (`parish_gold_loader.py`, `institution_dimension_sync.py`, `warehouse_worker.py`, `education_financial_sync.py`, `liturgical_calendar_loader.py`). New ETL/sync code must use `get_etl_pool()`/`discard_etl_pool()`, never `get_pool()`/`discard_pool()` — a slow or stuck ETL write once starved the dashboard's read pool by sharing it (see `_singleflight.py`'s docstring and this file's own connection-pool comments for the incident).
- Expensive, diocese-wide, identical-for-every-caller endpoints (`parish_cluster.py`, `cluster_forecast.py`, `health_scoring.py`'s batch endpoint, `financial_trend.py`'s unscoped trend) are wrapped in `_singleflight.coalesce()` so concurrent duplicate requests (e.g. multiple browser tabs on the same filter state) share one in-flight computation instead of each independently hitting RDS. Any new endpoint with this same shape (no/coarse parameters, hit on every dashboard load) should use the same pattern.
