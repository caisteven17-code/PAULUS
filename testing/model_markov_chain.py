...
def predict_next_state(current_state: int, T: np.ndarray) -> int:
    """Predict most likely next state from transition matrix."""
    return int(np.argmax(T[current_state]))
...
def train(df: pd.DataFrame) -> None:
    ...
    # Evaluate: predict next state from current for all pairs
    y_true = [t[1] for t in all_transitions]
    y_pred = [predict_next_state(t[0], T_global) for t in all_transitions]
