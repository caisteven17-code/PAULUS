...
export class ProjectService {
  ...
  async getProjects(entityId?: string, entityType?: string): Promise<Project[]> {
    let query = this.supabaseService.supabaseServer.from('projects').select('*').order('created_at', { ascending: false });
    ...
    if (error) { console.error('[project.service] getProjects error:', error.message); return []; }
  ...
  async saveProject(project: Project): Promise<Project> {
    ...
    const { data, error } = await this.supabaseService.supabaseServer
      ...
      .select()
      .single();
    if (error || !data) { console.error('[project.service] saveProject error:', error?.message); return project; }
  ...
  async getDonations(projectId?: string): Promise<Donation[]> {
    let query = this.supabaseService.supabaseServer.from('donations').select('*').order('created_at', { ascending: false });
    if (projectId) query = query.eq('project_id', projectId);
    const { data, error } = await query;
    if (error) { console.error('[project.service] getDonations error:', error.message); return []; }
  ...
  async saveDonation(donation: Donation): Promise<Donation> {
    ...
    const { data, error } = await this.supabaseService.supabaseServer
      ...
      .select()
      .single();
    if (error || !data) { console.error('[project.service] saveDonation error:', error?.message); return donation; }
    return this.toDonation(data);
  }

  async getExpenses(projectId?: string): Promise<ProjectExpense[]> {
    let query = this.supabaseService.supabaseServer.from('project_expenses').select('*').order('created_at', { ascending: false });
    if (projectId) query = query.eq('project_id', projectId);
    const { data, error } = await query;
    if (error) { console.error('[project.service] getExpenses error:', error.message); return []; }
  ...
  async saveExpense(expense: ProjectExpense): Promise<ProjectExpense> {
    ...
    const { data, error } = await this.supabaseService.supabaseServer
      ...
      .select()
      .single();
    if (error || !data) { console.error('[project.service] saveExpense error:', error?.message); return expense; }
