...
export class FinancialService {
  ...
  private generateRecords(
    ...
    return YEARS.flatMap((year) =>
      DEFAULT_RECORDS.map((r, i) => {
        ...
        if (this.pseudoRandom(ms + 500) > 0.90) {
          if (this.pseudoRandom(ms + 600) > 0.5) colVar *= 1.7; else disVar *= 2.0;
        }

        const collections           = Math.round(r.collections           * baseMultiplier * colVar * yMult);
        const consumableCollections = Math.round(r.consumableCollections * baseMultiplier * sustainabilityFactor * (0.8 + this.pseudoRandom(ms + 200) * 0.4) * yMult);
  ...
  async getRecords(
    ...
    const { error: insertError } = await this.supabaseService.supabaseServer
      .from('financial_records')
      .insert(rows);
  ...
  async deleteRecord(id: string): Promise<void> {
    const { error } = await this.supabaseService.supabaseServer
      .from('financial_records')
      .delete()
      .eq('id', id);
