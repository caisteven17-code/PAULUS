...
export function TopNav({
  ...
  if (role === 'priest' || role === 'school' || role === 'seminary') {
    ...
    const entityName = user?.entityName?.toUpperCase() || (
      role === 'school'
    ...
    const pastorName = user?.displayName || (
      role === 'school'
        ? 'Rev. Fr. John Doe'
        : role === 'seminary'
          ? 'Rev. Fr. James Smith'
    ...
    const userInitial = pastorName.charAt(0).toUpperCase() || (
      role === 'school' ? 'S' : role === 'seminary' ? 'R' : 'B'
    );
