...
export function UserRoleControl({ roles, onUpdateRoles, accounts = [] }: UserRoleControlProps) {
    ...
    return roles.map((role) => {
      ...
      if (!activeViewingKey) {
        if (role.id.includes('parish')) activeViewingKey = 'view_parish';
          ...
          role.id.includes('school') ||
          role.id.includes('principal') ||
          role.id.includes('supervisor') ||
          role.id.includes('officer')
        ) {
      ...
      if ((permissions as any)['manage_own_institution'] === undefined) {
        const isSingleInstitution =
          activeViewingKey === 'view_parish' ||
          activeViewingKey === 'view_seminary' ||
          activeViewingKey === 'view_school';
  ...
  const saveChanges = () => {
    ...
    const isDuplicate = sanitizedRoles.some((r) => r.id !== tempRole.id && r.name.trim().toLowerCase() === nameLower);
    if (isDuplicate) {
  ...
  const togglePermission = (permId: string) => {
    ...
    if (isViewingPerm) {
      ...
      ['view_diocese', 'view_parish', 'view_seminary', 'view_school', 'view_school_cluster', 'view_school_all'].forEach(
        (id) => {
      ...
      updatedPermissions['manage_announcements'] =
        permId === 'manage_announcements' ? !updatedPermissions['manage_announcements'] : false;
      updatedPermissions['view_announcements'] =
        permId === 'view_announcements' ? !updatedPermissions['view_announcements'] : false;
      ...
      const updatedPermissions = { ...tempRole.permissions } as any;
      updatedPermissions['manage_projects'] =
        permId === 'manage_projects' ? !updatedPermissions['manage_projects'] : false;
  ...
  const handleSaveNewRole = (e: React.FormEvent) => {
    ...
    const isDuplicate = sanitizedRoles.some((r) => r.name.trim().toLowerCase() === nameLower);
    if (isDuplicate) {
      setValidationError(`A role with the name "${trimmedName}" already exists.`);
      return;
    }
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 border border-gray-100">
            <div className="p-8 text-center space-y-6">
              ...
              <div className="space-y-2">
                <h3 className="text-2xl font-bold text-gray-900">{isAssigned ? 'Role In Use' : 'Delete Role'}</h3>
                <p className="text-sm text-gray-500 leading-relaxed text-center">
                  {isAssigned ? (
                    <>
                      Cannot delete the <span className="font-bold text-gray-900">"{selectedRole.name}"</span> role
                      because it is currently assigned to{' '}
                      <span className="font-bold text-gray-900">{assignedAccounts.length} user account(s)</span>:
                    ...
                    <>
                      ...
                      <span className="font-bold text-gray-900">"{selectedRole.name}"</span> role? This action cannot be
                      undone.
                    </>
                  )}
                </p>
                ...
                <div className="space-y-6">
                  ...
                  {(() => {
                    ...
                    const isSchoolSelected =
                      permissionsObj.view_school === true ||
                      permissionsObj.view_school_cluster === true ||
                      permissionsObj.view_school_all === true;
                    ...
                    const selectLevel = (levelId: string) => {
                      ...
                      updated['view_school_dashboard'] =
                        levelId === 'view_diocese' ||
                        levelId === 'view_school' ||
                        levelId === 'view_school_cluster' ||
                        levelId === 'view_school_all';
                      ...
                      if (levelId !== 'view_diocese') {
                        RESTRICTED_IDS.forEach((id) => {
                          if ((levelId === 'view_parish' || levelId === 'view_seminary') && id === 'view_priests')
                            return;
                          updated[id] = false;
                        });
                      }
                    ...
                    const levelCards = [
                      {
                        id: 'view_diocese',
                        label: 'Diocesan Level',
                        desc: 'Full access across the entire diocese.',
                        icon: '🏛️',
                      ...
                      },
                      {
                        id: 'view_parish',
                        label: 'Parish Level',
                        desc: 'Access limited to an assigned parish.',
                        icon: '⛪',
                      ...
                      {
                        id: 'view_seminary',
                        label: 'Seminary Level',
                        desc: 'Access limited to an assigned seminary.',
                        icon: '📖',
                      ...
                      },
                      {
                        id: 'view_school',
                        label: 'School Level',
                        desc: 'Access limited to an assigned school or cluster.',
                        icon: '🏫',
                    ...
                    interface DashSwitch {
                      id: string;
                      name: string;
                      desc: string;
                    }
                    const dashSwitches: DashSwitch[] = [];
                    if (isDioceseSelected || isParishSelected)
                      dashSwitches.push({
                        id: 'view_parish_dashboard',
                        name: 'Parish Dashboard',
                        desc: 'Allow viewing parish-level financial summaries and operational reports.',
                      });
                    if (isDioceseSelected || isSeminarySelected)
                      dashSwitches.push({
                        id: 'view_seminary_dashboard',
                        name: 'Seminary Dashboard',
                        desc: 'Allow viewing seminary-level financial summaries and educational timelines.',
                      });
                    if (isDioceseSelected || isSchoolSelected)
                      dashSwitches.push({
                        id: 'view_school_dashboard',
                        name: 'School Dashboard',
                        desc: 'Allow viewing school-level financial summaries and academic metrics.',
                      });
                    ...
                    if (anyLevelSelected) {
                      const dataItems: PermItem[] = [
                        {
                          ...
                          desc: 'Download blank CSV templates for data entry.',
                        },
                        {
                          id: 'upload_csv_entity',
                          name: 'Upload Entity CSV',
                          desc: 'Upload updated CSVs for their specific entity.',
                        },
                      ];
                      if (isDioceseSelected)
                        dataItems.push({
                          ...
                          desc: 'Upload and process master CSV templates for the diocese.',
                        });
                      ...
                      const projectItems: PermItem[] = [
                        { id: 'view_projects', name: 'View Projects', desc: 'View project lists and details.' },
                        {
                          ...
                          desc: 'Create, edit, and manage diocesan and entity projects.',
                        },
                      ];
                      ...
                      if (isDioceseSelected || isParishSelected || isSeminarySelected) {
                        const priestItems: PermItem[] = [
                          {
                            id: 'view_priests',
                            name: 'View Priest Profiles',
                            desc: 'View priest health trackers, assignments, and personnel dashboards.',
                          },
                        ];
                        if (isDioceseSelected)
                          priestItems.push({
                            ...
                            desc: 'Launch scenario planning and simulate clergy assignments.',
                          });
                      ...
                      const announcementItems: PermItem[] = [
                        {
                          ...
                          desc: 'View announcements and news bulletins.',
                        },
                      ];
                      if (isDioceseSelected || isParishSelected || isSeminarySelected) {
                        announcementItems.push({
                          ...
                          desc: 'Create, edit, and publish announcements.',
                        });
                      }
                          ...
                          items: [
                            {
                              id: 'digital_twin',
                              name: 'Digital Twin',
                              desc: 'Launch scenario simulations and mirror other institution dashboards.',
                            },
                            {
                              id: 'manage_entities',
                              name: 'Manage Entities',
                              desc: 'Manage and configure diocesan institutions, parishes, and schools.',
                            },
                          ],
                        });
                        permGroups.push({
                          ...
                          items: [
                            {
                              ...
                              desc: 'Add, update, or remove staff accounts in the system.',
                            },
                            {
                              id: 'manage_roles',
                              name: 'Manage User Roles',
                              desc: 'Change what each staff member is allowed to see and do in the system.',
                            },
                            {
                              id: 'view_audit_logs',
                              name: 'View Activity History',
                              desc: 'See a record of all actions taken by staff members in the system.',
                            },
                          ],
                        });
                      }
                    ...
                    return (
                      <div className="space-y-7">
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center gap-2">
                            <span className="w-5 h-5 rounded-full bg-[#1A1A1A] text-white text-[10px] font-bold flex items-center justify-center shrink-0">
                              1
                            </span>
                              ...
                              return (
                                <button
                                  ...
                                  className={`flex items-start gap-3 p-4 rounded-2xl border-2 text-left transition-all ${
                                    sel
                                      ? `${card.colorBg} ${card.colorRing} ring-2`
                                      : 'border-gray-100 hover:border-gray-200 bg-gray-50/50'
                                  }`}
                                >
                                  <span className="text-xl mt-0.5">{card.icon}</span>
                                  <div className="flex-1">
                                    <div className={`text-sm font-bold ${sel ? card.colorText : 'text-gray-700'}`}>
                                      {card.label}
                                    </div>
                                    <div className="text-[11px] text-gray-400 mt-0.5 leading-snug">{card.desc}</div>
                                  </div>
                                  {sel && (
                                    <span className={`mt-1.5 w-2.5 h-2.5 rounded-full shrink-0 ${card.colorDot}`} />
                                  )}
                                </button>
                              );
                            })}
                        ...
                        {(isParishSelected ||
                          isSeminarySelected ||
                          (isSchoolSelected && permissionsObj.view_school === true)) && (
                          <div className="space-y-3">
                            <div className="flex items-center gap-2">
                              <span className="w-5 h-5 rounded-full bg-[#1A1A1A] text-white text-[10px] font-bold flex items-center justify-center shrink-0">
                                1.5
                            ...
                            <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden divide-y divide-gray-50 shadow-sm">
                              {[
                                {
                                  val: true,
                                  name: `Manage ${isParishSelected ? 'Parish' : isSeminarySelected ? 'Seminary' : 'School'} Tab`,
                                  desc: `Allows editing contact number and email.`,
                                },
                                {
                                  val: false,
                                  name: `View Only ${isParishSelected ? 'Parish' : isSeminarySelected ? 'Seminary' : 'School'} Tab`,
                                  desc: `Restricts to read-only view.`,
                                },
                              ].map((opt) => {
                        ...
                        {isSchoolSelected && (
                          <div className="ml-2 pl-4 border-l-2 border-purple-200 space-y-2">
                            <div className="text-[10px] font-bold text-purple-500 uppercase tracking-widest mb-2">
                              School Access Scope
                            </div>
                            {[
                              {
                                id: 'view_school',
                                name: 'Assigned School Only',
                                desc: 'View records specific to one assigned school.',
                              },
                              {
                                id: 'view_school_cluster',
                                name: 'Cluster of Schools',
                                desc: 'View records for an assigned cluster of schools.',
                              },
                              {
                                id: 'view_school_all',
                                name: 'All Schools',
                                desc: 'View records for all schools in the diocese.',
                              },
                            ].map((sub) => {
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="bg-amber-50/40 border border-amber-100 rounded-2xl p-4 space-y-3">
                              <p className="text-[11px] text-amber-700 font-medium">
                                Toggle which dashboards this role can access.
                              </p>
                              {dashSwitches.map((sw) => {
                                ...
                                return (
                                  <div
                                    key={sw.id}
                                    className="flex items-center justify-between gap-4 py-2.5 border-b border-amber-100/60 last:border-0"
                                  >
                                    <div>
                                    ...
                                    >
                                      <span
                                        className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${isToggled ? 'translate-x-4' : 'translate-x-0'}`}
                                      />
                                    </button>
                                  </div>
                                );
        ...
        <div className="px-10 py-8 border-b border-gray-200 flex-shrink-0 bg-white z-10 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {validationError && isEditing && (
              <span className="text-rose-500 text-sm font-bold animate-in fade-in mr-3">{validationError}</span>
            )}
            {!isPredefined && (
              <>
                {!isEditing ? (
        ...
        <div className="flex-1 overflow-y-auto p-10 bg-gray-50/50">
          <div className="max-w-4xl space-y-12">
            {(() => {
              ...
              const isSchoolSelected =
                activePermissionsState.view_school === true ||
                activePermissionsState.view_school_cluster === true ||
                activePermissionsState.view_school_all === true;
              const RESTRICTED_IDS = [
                'digital_twin',
                'upload_csv_admin',
                'manage_entities',
                ...
                <>
                  {filteredPermissions.map((category, idx) => (
                    <React.Fragment key={idx}>
                      <div>
                        ...
                        <div className="bg-white border border-gray-100 rounded-[24px] overflow-hidden divide-y divide-gray-50 shadow-sm">
                          {category.permissions.map((perm) => {
                            ...
                            const isEnabled =
                              isViewingCategory && perm.id === 'view_school'
                                ? activePermissionsState.view_school === true ||
                                  activePermissionsState.view_school_cluster === true ||
                                  activePermissionsState.view_school_all === true
                              ...
                              <React.Fragment key={perm.id}>
                                ...
                                >
                                  ...
                                  {isViewingCategory ? (
                                    ...
                                    >
                                      {isEnabled && <span className="w-2.5 h-2.5 rounded-full bg-[#22C55E]" />}
                                    </button>
                                  ) : (
                                    <button
                                      type="button"
                                      ...
                                      className={`relative inline-flex h-6 w-11 flex-shrink-0 rounded-full border-2 border-transparent transition-all duration-300 ease-in-out focus:outline-none ${
                                        !canEdit
                                          ? 'cursor-not-allowed'
                                          : 'cursor-pointer focus:ring-4 focus:ring-[#22C55E]/20 focus:ring-offset-0'
                                    ...
                                    {[
                                      {
                                        ...
                                        name: 'Assigned School Only',
                                        description:
                                          'Allows the user to view records specific to their assigned school.',
                                      },
                                      {
                                        id: 'view_school_cluster',
                                        name: 'Cluster School',
                                      ...
                                      const isSubChecked =
                                        activePermissionsState[sub.id as keyof typeof activePermissionsState] === true;
                                      return (
                                        <label
                                          key={sub.id}
                                          onClick={() => {
                      ...
                      {category.category === 'Access Level' &&
                        (isDioceseSelected || isParishSelected || isSeminarySelected || isSchoolSelected) &&
                        (() => {
                          const dashItems = [
                            ...(isDioceseSelected || isParishSelected
                              ? [
                                  {
                                    id: 'view_parish_dashboard',
                                    name: 'Parish Dashboard',
                                    description:
                                      'Allows viewing and accessing parish-level financial summaries and operational reports.',
                                  },
                                ]
                              : []),
                            ...(isDioceseSelected || isSeminarySelected
                              ? [
                                  {
                                    id: 'view_seminary_dashboard',
                                    name: 'Seminary Dashboard',
                                    description:
                                      'Allows viewing and accessing seminary-level financial summaries and educational timelines.',
                                  },
                                ]
                              : []),
                            ...(isDioceseSelected || isSchoolSelected
                              ? [
                                  {
                                    id: 'view_school_dashboard',
                                    name: 'School Dashboard',
                                    description:
                                      'Allows viewing and accessing school-level financial summaries and academic metrics.',
                                  },
                                ]
                              : []),
                          ];
                          ...
                          return (
                            <div>
                              ...
                              <div className="bg-white border border-gray-100 rounded-[24px] overflow-hidden divide-y divide-gray-50 shadow-sm">
                                {dashItems.map((sw) => {
                                  const isToggled =
                                    activePermissionsState[sw.id as keyof typeof activePermissionsState] === true;
                                  return (
                                    <div
                                      key={sw.id}
                                      onClick={() => {
                                        if (canEditDash && tempRole)
                                          setTempRole({
                                            ...tempRole,
                  ...
                  {(isParishSelected ||
                    isSeminarySelected ||
                    (isSchoolSelected && activePermissionsState.view_school === true)) &&
                    (() => {
                        ...
                        <div className="mt-8">
                          ...
                          <div className="bg-white border border-gray-100 rounded-[24px] overflow-hidden divide-y divide-gray-50 shadow-sm">
                            {[
                              {
                                val: true,
                                name: `Manage ${instLabel} Tab`,
                                description: `Allows editing contact number and email in the My ${instLabel} tab.`,
                              },
                              {
                                val: false,
                                name: `View Only ${instLabel} Tab`,
                                description: `Restricts the My ${instLabel} tab to read-only view.`,
                              },
                            ].map((opt) => {
