...
export const FinancialHealthGauge: React.FC<FinancialHealthGaugeProps> = ({
  ...
  const finalZoneName =
    zoneName || (clampedScore < 33 ? 'Critical Zone' : clampedScore < 66 ? 'Caution Zone' : 'Optimal Zone');
  ...
  return (
    <div
      className={`flex flex-col items-center text-center w-full max-w-[320px] mx-auto bg-white rounded-[48px] p-8 shadow-sm border border-slate-100 ${className}`}
    >
      {/* Element 1 (Top): The Gauge Arc */}
      <div className="relative overflow-visible mb-[48px]" style={{ width: size, height: centerY + 15 }}>
        <svg width={size} height={centerY + 15} viewBox={`0 0 ${size} ${centerY + 15}`} className="overflow-visible">
          <defs>
      ...
      <motion.div
        ...
        className="mb-[24px]"
      >
        <span className="text-6xl font-black tracking-tighter leading-none m-0 p-0" style={{ color: currentColor }}>
          {clampedScore}
        </span>
      </motion.div>
      ...
      >
        ...
        <span className="text-xs font-bold text-white tracking-tight uppercase">{finalZoneName}</span>
      </motion.div>
          ...
          {typeof description === 'string'
            ? description.split(finalZoneName).map((part, i, arr) => (
                <React.Fragment key={i}>
                  {part}
                  {i < arr.length - 1 && (
                    <span style={{ color: currentColor, fontWeight: 'bold' }}>{finalZoneName}</span>
                  )}
                </React.Fragment>
              ))
            : description}
