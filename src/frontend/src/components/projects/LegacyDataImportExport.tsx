...
export function LegacyDataImportExport({
  entityName,
  entityType,
  year,
  onImport,
  ...
  const getSubmissionStatus = useCallback(() => {
    ...
    if (
      monthsLate >= SUBMISSION_CONFIG.MONTHS_LATE_WARNING &&
      monthsLate <= SUBMISSION_CONFIG.MONTHS_LATE_WARNING_MAX
    ) {
      return {
        status: 'warning',
        color: 'bg-amber-50 border-amber-200',
        badge: `Warning (${monthsLate}-${SUBMISSION_CONFIG.MONTHS_LATE_WARNING_MAX} months late)`,
  ...
  const downloadTemplate = useCallback(() => {
    ...
    const csvHeader =
      'Month,Collections (PHP),Consumable Collections (PHP),Disbursements (PHP),Sacraments Rate (%),Sacraments Arancel (PHP),Sacraments Parish Share (PHP),Sacraments Over & Above (PHP),Collections Mass (PHP),Collections Other (PHP),Collections Other Receipts (PHP),Expenses Pastoral (PHP),Expenses Parish (PHP),Net Receipts (PHP),Mass Intentions Not Claimed (PHP),Mass Intentions Claimed (PHP),Special Collections (PHP),Pastoral Fund Total Net Receipts (PHP)\n';
    ...
    <div className="space-y-3 md:space-y-5">
      {importStatus === 'success' && (
        <motion.div
          ...
          className="flex items-center gap-3 rounded-lg border border-green-200 bg-green-50 p-3 text-green-700 md:p-4"
        >
      ...
      {importStatus === 'error' && (
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="flex items-center gap-3 rounded-lg border border-red-200 bg-red-50 p-3 text-red-700 md:p-4"
        >
      ...
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        className={`rounded-lg border p-4 md:p-5 ${submissionStatus.color}`}
      >
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <Clock className={`h-4 w-4 flex-shrink-0 md:h-5 md:w-5 ${submissionStatus.textColor}`} />
              <span className={`text-xs font-bold md:text-sm ${submissionStatus.textColor}`}>
            ...
            <div className="space-y-1 text-[11px] md:text-sm">
              ...
              {lastSubmissionDate ? (
                <p className={submissionStatus.textColor}>
                  ...
                  {new Date(lastSubmissionDate).toLocaleDateString('en-PH', {
                    month: 'short',
                    day: 'numeric',
                    year: 'numeric',
                  })}
      ...
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-base"
      >
          ...
          {budgetAnnual > 0 ? (
            <>
              <div className="flex items-baseline justify-between gap-3">
                ...
                <span className="text-xl font-bold text-gold-600 md:text-2xl">
            ...
            <p className="text-xs italic text-gray-500 md:text-sm">
              No budget set yet. Use the button below to add one.
            </p>
          )}
          <button
            onClick={() => setIsBudgetOpen(true)}
            className="w-full rounded-lg bg-gold-500 px-4 py-2 text-xs font-bold text-black transition-colors hover:bg-gold-600 md:py-2.5 md:text-sm"
          >
      ...
      <div className="flex flex-col gap-2 sm:flex-row md:gap-3">
        <button
          ...
          className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-gold-500 px-4 py-3 text-xs font-bold text-black shadow-md transition-all hover:scale-[1.01] hover:bg-gold-600 md:text-sm"
        >
        ...
        <button
          ...
          className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-emerald-700 px-4 py-3 text-xs font-bold text-white shadow-md transition-all hover:scale-[1.01] hover:bg-emerald-800 md:text-sm"
        >
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          ...
          >
            ...
            <p className="mb-6 text-xs text-gray-600 md:text-sm">
              Select a CSV file with financial data for {entityName}. Download the template first to confirm the
              required format.
            </p>
            <div className="space-y-4">
              <button
                onClick={() => fileInputRef.current?.click()}
                className="w-full cursor-pointer rounded-lg border-2 border-dashed border-gold-400 p-8 text-center transition-colors hover:bg-gold-50"
              >
              ...
              <button
                ...
                className="w-full rounded-lg bg-gray-200 px-4 py-2.5 text-sm font-medium text-gray-900 transition-colors hover:bg-gray-300"
              >
                Cancel
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex gap-2 rounded-lg bg-gray-100 p-2">
                <button
                  ...
                  className={`flex-1 rounded-lg px-3 py-2 text-sm font-bold transition-colors ${budgetInputType === 'annual' ? 'bg-gold-500 text-black' : 'text-gray-700 hover:bg-gray-200'}`}
                >
                  Annual Budget
                </button>
                <button
                  onClick={() => setBudgetInputType('monthly')}
                  className={`flex-1 rounded-lg px-3 py-2 text-sm font-bold transition-colors ${budgetInputType === 'monthly' ? 'bg-gold-500 text-black' : 'text-gray-700 hover:bg-gray-200'}`}
                >
                  Monthly Budget
                </button>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 rounded-lg bg-gray-200 px-4 py-2 font-bold text-gray-900 transition-colors hover:bg-gray-300"
                >
                  Cancel
                </button>
                <button
                  onClick={handleBudgetSave}
                  className="flex-1 rounded-lg bg-gold-500 px-4 py-2 font-bold text-black transition-colors hover:bg-gold-600"
                >
                  Save Budget
                </button>
              </div>
