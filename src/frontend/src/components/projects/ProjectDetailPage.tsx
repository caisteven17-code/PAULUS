...
export function ProjectDetailPage({ project, donations, expenses, onBack, onAddDonation, onAddExpense, onCloneProject, role, canManageProjects }: ProjectDetailPageProps) {
  const [isDonationModalOpen, setIsDonationModalOpen] = useState(false);
  const [isExpenseModalOpen, setIsExpenseModalOpen] = useState(false);
  const [historicalRecords, setHistoricalRecords] = useState<FinancialRecord[]>([]);
  const progress = (project.currentAmount / project.targetAmount) * 100;
  const canRecordProjectEntries = canManageProjects !== undefined
    ? canManageProjects
