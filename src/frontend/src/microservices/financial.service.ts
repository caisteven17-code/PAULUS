...
function generateRecords(
  ...
  return YEARS.flatMap((year) =>
    DEFAULT_RECORDS.map((r, i) => {
      ...
      if (pseudoRandom(ms + 500) > 0.9) {
        if (pseudoRandom(ms + 600) > 0.5) colVar *= 1.7;
        else disVar *= 2.0;
      }

      const collections = Math.round(r.collections * baseMultiplier * colVar * yMult);
      const consumableCollections = Math.round(
        r.consumableCollections * baseMultiplier * sustainabilityFactor * (0.8 + pseudoRandom(ms + 200) * 0.4) * yMult,
      );
...
export const financialService = {
  ...
  async getRecords(
    ...
    const { error: insertError } = await supabaseServer.from('financial_records').insert(rows);
  ...
  async getAllRecords(): Promise<FinancialRecord[]> {
    ...
    const { error: bulkErr } = await supabaseServer.from('financial_records').insert(allGenerated.map(fromRecord));
  ...
  async saveRecord(record: FinancialRecord): Promise<FinancialRecord> {
    ...
    const { data, error } = await supabaseServer.from('financial_records').upsert(row).select().single();
  ...
  async deleteRecord(id: string): Promise<void> {
    const { error } = await supabaseServer.from('financial_records').delete().eq('id', id);
