...
export const projectService = {
  ...
  async getProjects(entityId?: string, entityType?: string): Promise<Project[]> {
    ...
    if (error) { console.error('[project.service] getProjects:', error.message); return []; }
  ...
  async saveProject(project: Project): Promise<Project> {
    ...
    const { data, error } = await supabaseServer
      ...
      .select()
      .single();
    if (error || !data) { console.error('[project.service] saveProject:', error?.message); return project; }
  ...
  async getDonations(projectId?: string): Promise<Donation[]> {
    ...
    const { data, error } = await query;
    if (error) { console.error('[project.service] getDonations:', error.message); return []; }
  ...
  async saveDonation(donation: Donation): Promise<Donation> {
    ...
    const { data, error } = await supabaseServer
      ...
      .select()
      .single();
    if (error || !data) { console.error('[project.service] saveDonation:', error?.message); return donation; }
  ...
  async getExpenses(projectId?: string): Promise<ProjectExpense[]> {
    ...
    if (projectId) query = query.eq('project_id', projectId);
    const { data, error } = await query;
    if (error) { console.error('[project.service] getExpenses:', error.message); return []; }
  ...
  async saveExpense(expense: ProjectExpense): Promise<ProjectExpense> {
    ...
    const { data, error } = await supabaseServer
      ...
      .select()
      .single();
    if (error || !data) { console.error('[project.service] saveExpense:', error?.message); return expense; }
