...
import { INITIAL_ROLES, VICARIATES, CLASSES, INITIAL_PARISHES, INITIAL_SEMINARIES, INITIAL_SCHOOLS, ALL_PARISHES } from '../constants';
...
export function Settings({ onBack, onLogout, onNavigate, role = 'bishop', initialTab }: SettingsProps) {
  ...
  const institutionNamePlaceholder = formState.institutionType
    ? (formState.institutionType === 'diocese' ? 'Diocese of San Pablo' : `Select ${institutionNameLabel.toLowerCase()}`)
  ...
  const handleSaveAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const constructedLeaderName = `${formState.firstName} ${formState.lastName}`.trim() || `${formState.entity} ${roleLabel}`;
      ...
      try {
        ...
        if (editingAccountId !== null) {
          const updated = stored.map(u => u.id.toString() === editingAccountId.toString() ? {
            ...u,
            email: formState.email,
            displayName: constructedLeaderName,
            role: accessRole,
  ...
  const handleEditClick = (account: any) => {
    ...
    const resolvedType = (account.entityType === 'diocese' || account.entityType === 'parish' || account.entityType === 'seminary' || account.entityType === 'school') 
      ? account.entityType 
      : '';
  ...
  const closeModal = () => {
    ...
    setShowInstitutionSuggestions(false);
    setFormState({ institutionType: '', entity: '', leader: '', firstName: '', lastName: '', email: '', role: '', password: '' });
  };
  ...
  const getFormattedFullName = (leaderName: string) => {
    ...
    if (lower.includes('school') || lower.includes('seminary') || lower.includes('parish') || lower.includes('cathedral') || lower.includes('diocese')) {
      return cleanName;
    }
              ...
              <div className="space-y-4">
                  ...
                  <select 
                    ...
                    onChange={(e) => {
                      ...
                      if (formState.institutionType === 'school') {
                        ...
                        } else if (formState.entity === 'Diocesan Schools' || formState.entity.startsWith('Cluster')) {
                          updatedEntity = '';
                        }
                      }
                ...
                <div>
                  ...
                  {formState.institutionType === 'diocese' ? (
                    ...
                    >
                      <option value="" disabled>{institutionNamePlaceholder}</option>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-gray-100">
                      <th className="pb-3.5 font-bold text-gray-400 text-[10px] uppercase tracking-widest w-1/5">Institution Name</th>
                      <th className="pb-3.5 font-bold text-gray-400 text-[10px] uppercase tracking-widest w-[15%]">Institution Type</th>
                      <th className="pb-3.5 font-bold text-gray-400 text-[10px] uppercase tracking-widest w-1/5">Full Name</th>
                      <th className="pb-3.5 font-bold text-gray-400 text-[10px] uppercase tracking-widest w-1/5">Email Address</th>
                      <th className="pb-3.5 font-bold text-gray-400 text-[10px] uppercase tracking-widest w-[15%]">Role</th>
                      <th className="pb-3.5 font-bold text-gray-400 text-[10px] uppercase tracking-widest text-right">Actions</th>
                    </tr>
                  </thead>
                    ...
                    {filteredAccounts.length > 0 ? (
                      filteredAccounts.map((account) => (
                        <tr key={account.id} className="group hover:bg-gray-50/50 transition-colors">
                          ...
                          <td className="py-4 pr-4 text-gray-600 text-sm font-medium capitalize">{account.entityType || 'Institution'}</td>
                          ...
                          <td className="py-4 text-right">
                            <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              ...
                              >
                                {viewMode === 'active' ? <Database className="w-3.5 h-3.5" /> : <ArrowRight className="w-3.5 h-3.5 rotate-180" />}
                              </button>
                            </div>
                          </td>
                        </tr>
