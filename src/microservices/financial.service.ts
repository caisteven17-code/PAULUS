/**
 * Financial Service — manages FinancialRecord persistence via Supabase.
 * Falls back to in-memory seeded data when a row does not yet exist in the DB.
 */

import type { FinancialRecord, EntityClass } from '../types';
import { ALL_PARISHES, INITIAL_SEMINARIES, INITIAL_SCHOOLS } from '../constants';
import { supabaseServer } from '../lib/supabase';
import { entityKey, newId } from './store';
...
// Known institution target scores (shapes seeded mock data)
...
// ------------------------------------------------------------------
// Supabase row ↔ FinancialRecord mappers
// ------------------------------------------------------------------
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function toRecord(row: any): FinancialRecord {
  return {
    id:                                     row.id,
    month:                                  row.month,
    ...
    collections_otherReceipts:              row.collections_other_receipts != null ? Number(row.collections_other_receipts) : undefined,
    ...
    others_massIntentionsNotClaimed:        row.others_mass_intentions_not_claimed != null ? Number(row.others_mass_intentions_not_claimed) : undefined,
    others_massIntentionsClaimed:           row.others_mass_intentions_claimed     != null ? Number(row.others_mass_intentions_claimed)     : undefined,
    others_specialCollections:              row.others_special_collections != null ? Number(row.others_special_collections) : undefined,
    pastoralParishFundTotalNetReceipts:     row.pastoral_parish_fund_total_net_receipts != null ? Number(row.pastoral_parish_fund_total_net_receipts) : undefined,
...
function fromRecord(r: FinancialRecord): Record<string, unknown> {
  return {
    id:                                         r.id ?? newId(),
    month:                                      r.month,
    year:                                       r.year ?? null,
...
// ------------------------------------------------------------------
// Mock data generation (used when Supabase has no data for an entity)
// ------------------------------------------------------------------
...
function generateRecords(
  ...
  let classBonus = 0;
  if (entityClass === 'Class A') { classBonus =  0.30; baseMultiplier *= 2.0; }
  if (entityClass === 'Class B') { classBonus =  0.15; baseMultiplier *= 1.4; }
  if (entityClass === 'Class D') { classBonus = -0.15; baseMultiplier *= 0.7; }
  if (entityClass === 'Class E') { classBonus = -0.30; baseMultiplier *= 0.4; }
  ...
  const fiestaMonth = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov'][
    Math.floor(pseudoRandom(seed + 999) * 11)
  ];

  const YEARS = [2024, 2025, 2026];
  // Year-over-year growth multipliers (slight variation per year)
  ...
  return YEARS.flatMap((year) =>
    DEFAULT_RECORDS.map((r, i) => {
      ...
      let colVar = 0.7 + pseudoRandom(ms) * 0.6;
      let disVar = 0.8 + pseudoRandom(ms + 100) * 0.4;
      ...
      if (pseudoRandom(ms + 500) > 0.90) {
        if (pseudoRandom(ms + 600) > 0.5) colVar *= 1.7; else disVar *= 2.0;
      }

      const collections           = Math.round(r.collections           * baseMultiplier * colVar * yMult);
      const consumableCollections = Math.round(r.consumableCollections * baseMultiplier * sustainabilityFactor * (0.8 + pseudoRandom(ms + 200) * 0.4) * yMult);
      ...
      return {
        id: newId(), month: r.month, year, entityId, entityType, entityClass,
        collections, consumableCollections, disbursements, netReceipts,
        sacraments_rate:                  Math.round(collections * 0.15),
        sacraments_arancel:               Math.round(collections * 0.10),
        sacraments_parishShare:           Math.round(collections * 0.08),
        sacraments_overAbove:             Math.round(collections * 0.05),
        collections_mass:                 Math.round(collections * 0.40),
        collections_other:                Math.round(collections * 0.12),
        collections_otherReceipts:        Math.round(collections * 0.10),
...
export const financialService = {
  /** Fetch records for one entity from Supabase; seed & persist if empty. */
  async getRecords(
    entityId: string,
    entityType: 'parish' | 'seminary' | 'school',
    entityClass?: EntityClass,
    ...
    if (error) {
      console.error('[financial.service] getRecords:', error.message);
      return generateRecords(entityId, entityType, entityClass);
    }

    if (data && data.length > 0) return data.map(toRecord);

    // No rows yet — generate, persist, return
    const generated = generateRecords(entityId, entityType, entityClass);
    const rows = generated.map(fromRecord);
    const { error: insertError } = await supabaseServer
      .from('financial_records')
      .insert(rows);
    if (insertError) console.error('[financial.service] seed insert:', insertError.message);
    return generated;
  },

  /** Fetch all records across every known entity. */
  async getAllRecords(): Promise<FinancialRecord[]> {
    ...
    if (error) {
      console.error('[financial.service] getAllRecords:', error.message);
      return [];
    }

    if (data && data.length > 0) return data.map(toRecord);

    // Seed every known entity then return all
    ...
    for (const e of entities) {
      const records = generateRecords(e.id, e.type, e.class);
      allGenerated.push(...records);
    }

    const { error: bulkErr } = await supabaseServer
      .from('financial_records')
      .insert(allGenerated.map(fromRecord));
    if (bulkErr) console.error('[financial.service] bulk seed:', bulkErr.message);
  ...
  async saveRecord(record: FinancialRecord): Promise<FinancialRecord> {
    ...
    if (error || !data) {
      console.error('[financial.service] saveRecord:', error?.message);
      return record;
    }
  ...
  async deleteRecord(id: string): Promise<void> {
    const { error } = await supabaseServer
      ...
      .delete()
      .eq('id', id);
    if (error) console.error('[financial.service] deleteRecord:', error.message);
  },

  /** Parse CSV text into FinancialRecord objects. */
  parseCSV(
    csvText: string,
    entityId = 'default',
  ...
  generateTemplateCSV(entityType = 'parish'): string {
    if (entityType === 'seminary') {
      return 'Month,Enrollment,Capacity,Staff,Collections,Disbursements\n' +
        DEFAULT_RECORDS.map(r => `${r.month},${Math.floor(Math.random()*50)},60,8,${r.collections},${r.disbursements}`).join('\n');
    }
    if (entityType === 'school') {
      return 'Month,Enrollment,Capacity,Staff,Level,Collections,Disbursements\n' +
        DEFAULT_RECORDS.map(r => `${r.month},${Math.floor(Math.random()*1000)},1500,45,K-12,${r.collections},${r.disbursements}`).join('\n');
    }
    if (entityType === 'diocese') {
      return 'Month,General Fund,Mission Fund,Cemetery Fund,Total Income,Total Expenses\n' +
        DEFAULT_RECORDS.map(r => `${r.month},${r.collections*0.6},${r.collections*0.2},${r.collections*0.2},${r.collections},${r.disbursements}`).join('\n');
    }
    return 'Month,Collections / Receipts,Consumable Collections,Disbursements\n' +
