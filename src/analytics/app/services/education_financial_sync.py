...
def _fetch_changes(config: EntityConfig) -> list[dict]:
    ...
    while True:
        ...
        rows.extend(
            row for row in batch if (str(row.get("updated_at") or ""), str(row["id"])) > (watermark_time, watermark_id)
        )
        if len(batch) < PAGE_SIZE:
            break
        offset += PAGE_SIZE
...
def sync_dimensions(config: EntityConfig) -> dict[str, int]:
    ...
    account_rows = [
        ...
        for row in accounts
        if row.get("deleted_at") is None and row.get("is_active", True)
    ]
    if account_rows:
        analytics_db.upsert_rows(
            config.analytics_schema,
            config.account_dimension_table,
            account_rows,
...
def _gold_payload(config: EntityConfig, source: dict, entity_key: int, date_key: int) -> dict:
    ...
    if config.entity_type == "school":
        values["total_inflow"] = sum(
            values[field] for field in ("tuition_revenues", "miscellaneous_fees", "other_income", "subsidy_inflow")
        )
        values["total_outflow"] = sum(
            values[field]
            for field in (
                "faculty_payroll",
                "admin_staff_payroll",
...
def sync_record(config: EntityConfig, source: dict) -> dict[str, int]:
    ...
    line_rows = [
        {
            ...
            "section_code": line.get("section_code"),
            "subsection_code": line.get("subsection_code"),
            "item_code": line.get("item_code"),
            "item_label": line["item_label"],
            "item_type": line.get("item_type"),
            "amount": line.get("amount") or 0,
            "source_updated_at": line.get("updated_at"),
            "source_deleted_at": line.get("deleted_at"),
        }
        for line in lines
        if line.get("deleted_at") is None
    ]
    if line_rows:
        analytics_db.upsert_rows(
            config.silver_schema,
    ...
    submission_key = _submission_key(source.get("submission_batch_id"))
    analytics_db.execute(
        f"DELETE FROM {config.analytics_schema}.{breakdown_table} WHERE {config.dimension_key}=%s AND date_key=%s",
        (entity_key, date_key),
        pool=analytics_db.get_etl_pool(),
    )
    if active:
