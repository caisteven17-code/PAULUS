...
Note    : Parishes with fewer than 4 months are skipped.
          STL decomposition (preferred) now requires >= 36 months; shorter
          series are summarized but not decomposed.
"""
...
from utils import load_data, engineer_features, print_metric_table, MODELS_DIR, safe_pct_change

warnings.filterwarnings("ignore")
from dashboard_graphs import export_model_dashboard_preview

MIN_PERIODS = 36   # Require 3 years of monthly data before trend isolation.
...
def train(df: pd.DataFrame, target: str = "total_receipts_final") -> None:
    ...
    for parish in parishes:
        ...
        # KPI: MoM change
        mom    = safe_pct_change(series).mean() * 100 if len(series) >= 2 else np.nan
...
if __name__ == "__main__":
    ...
    train(df)
    export_model_dashboard_preview("Time Series Decomposition", df)
