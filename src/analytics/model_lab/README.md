...
This folder was moved from the repository-level `testing/` model sandbox into
`src/analytics/model_lab/` so the analytics package carries the model training
scripts, dashboard test harness, saved `.joblib` artifacts, source archives, and
generated output references together.

These modules are intentionally not imported by `src/analytics/main.py`.
Several scripts depend on optional heavy libraries and local workbook/artifact
files, so loading them at FastAPI startup would make the analytics service more
fragile. Promote a model into `app/services/{descriptive,diagnostic,predictive,prescriptive}/`
only after its function signature and required data contract are clear.
...
testing/
  ...
  2023.xlsx, 2024.xlsx, 2025.xlsx     ← source Excel workbooks
...
Dataset source: place the three Excel workbooks `2021.xlsx`, `2022.xlsx`, and
`2023.xlsx` in this folder. Each workbook should have monthly tabs such as
`January`, `February`, or `January 2023`. The shared loader combines all monthly
...
Open `2023.xlsx, 2024.xlsx, and 2025.xlsx` and fill in the financial columns.
The models read these Excel workbooks directly from this folder.
...
### Dashboard graph model test harness

Use this before the actual financial/project data is complete. It creates a
synthetic Diocese dataset, runs several representative model families, and
exports graph assets that match the current dashboard visual design.
...
Outputs are written to `testing/outputs/dashboard_graphs/`:

| File | Purpose |
|---|---|
| `forecast_comparison.png` / `.svg` | Historical vs forecast line chart styled like the Bishop dashboard |
| `model_leaderboard.png` / `.svg` | Champion model comparison using WAPE |
| `feature_importance.png` / `.svg` | Top forecast drivers for explainability cards |
| `project_risk_probabilities.png` / `.svg` | Logistic project risk probabilities |
| `parish_cluster_map.png` / `.svg` | Parish segmentation preview |
| `anomaly_timeline.png` / `.svg` | Isolation Forest anomaly preview |
| `model_test_summary.csv` | Model metric table for quick comparison |
| `synthetic_financial_testing_data.csv` | Mock monthly parish data used by the test harness |
| `synthetic_project_testing_data.csv` | Mock project data used by the test harness |

The styling lives in `dashboard_graphs.py`: church green `#1a472a`, gold
`#D4AF37`, white cards, gray gridlines, and compact labels matching the
frontend chart language.

Each individual `model_*.py` file also exports its own dashboard preview graph
after it runs. Those per-model images are written to
`testing/outputs/model_graphs/`. If the annual Excel workbooks are not present,
`utils.py` automatically supplies synthetic model-testing data so the scripts can
still run before actual records are ready.
