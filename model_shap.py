...
def build_feature_frame(df: pd.DataFrame, features: list[str]) -> pd.DataFrame:
    """Recreate the encoded XGBoost feature matrix and align it to the artifact."""
    base_features = [feature for feature in features if not feature.startswith("parish_name_")]
    required_columns = ["parish_name"] + base_features
    missing = [column for column in required_columns if column not in df.columns]
    if missing:
        raise KeyError(f"Missing columns needed for SHAP feature matrix: {missing}")
    ...
    return X.reindex(columns=features, fill_value=0).fillna(0)


def unwrap_xgboost_model(model):
    """Return the fitted tree model inside sklearn target transformers."""
...
def explain(df: pd.DataFrame) -> None:
    ...
    X = build_feature_frame(data, features)
    ...
    tree_model  = unwrap_xgboost_model(model)
...
if __name__ == "__main__":
    ...
    explain(df)
    export_model_dashboard_preview("SHAP", df)
