-- Expose only the final Mass Collections measure. Gross, rate, and tax amount
-- remain implementation details of the formula and validator.
...
ALTER TABLE parish_analytics.fact_parish_monthly_financials
  DROP COLUMN IF EXISTS collections_mass_gross,
  DROP COLUMN IF EXISTS collections_mass_tax_rate,
  DROP COLUMN IF EXISTS collections_mass_tax_amount;

COMMENT ON VIEW parish_analytics.vw_parish_monthly_financial_candidates IS
  'Read-only parish_monthly_v3 candidates exposing net Mass and Other Collections measures only.';
