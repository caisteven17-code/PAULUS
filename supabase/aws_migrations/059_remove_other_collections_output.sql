-- Remove the standalone Other Collections net measure while retaining its
-- contribution to total_collections.
...
ALTER TABLE parish_analytics.fact_parish_monthly_financials
  DROP COLUMN IF EXISTS collections_other_net;

COMMENT ON VIEW parish_analytics.vw_parish_monthly_financial_candidates IS
  'Read-only parish_monthly_v3 candidates; Other Collections remains included internally in total_collections.';
